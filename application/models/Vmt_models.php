...
class Vmt_models extends CI_Model {
    ...
    public function getJobYardManager($data){
		...
		$qryYardManager = "SELECT * FROM (SELECT 
				...
				ROUND(E.WEIGHT/1000,1) WEIGHTREAL,
				E.GT_JS_BLOCK,
				E.GT_JS_BLOCK_NAME,
	...
	public function validationTierContainer($ydYard, $ydBlock, $ydSlot, $ydRow, $ydBlockName, $tier){
		// $q = "SELECT MAX(YD_TIER) MAX_YD_TIER, NO_CONTAINER FROM CON_LISTCONT WHERE ACTIVE='Y' AND ID_OP_STATUS IN ('YSY','YGY','QYG','YGR','YSR','YYR') AND YD_YARD='$ydYard' AND YD_BLOCK='$ydBlock' AND YD_SLOT='$ydSlot' AND YD_ROW='$ydRow' AND YD_BLOCK_NAME='$ydBlockName' GROUP BY NO_CONTAINER ORDER BY MAX_YD_TIER DESC";
		// $qryYardManager = "SELECT 
			...
			// AND E.GT_JS_BLOCK IN(SELECT ID_BLOCK FROM M_YARDBLOCK WHERE ID_YARD = ".$this->gtools->g_session('yard').")
			// AND A.IS_BYPASS='0'
			// AND E.YD_YARD = '$ydYard' 
			// AND E.YD_BLOCK = '$ydBlock' 
			// AND E.YD_SLOT = '$ydSlot' 
			// AND E.YD_ROW = '$ydRow' 
			// AND E.YD_BLOCK_NAME = '$ydBlockName' 
			// AND A.ID_TERMINAL = ".$this->gtools->g_session('terminal')."
			// AND NOT (
				// ( A.EVENT = 'O' AND A.STATUS_FLAG = 'C' ) 
				// OR ( A.EVENT = 'P' AND A.STATUS_FLAG = 'C' ) 
			// ) ";
		$qryYardManager = "SELECT 
				CASE WHEN (A.ID_OP_STATUS = 'YYY' AND EVENT = 'P' AND A.STATUS_FLAG = 'P') 
					THEN E.GT_JS_TIER
