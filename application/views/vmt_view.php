...
<style>
	...
	.jobclass {
		padding:5px;min-width:30px;display: inline-block;text-align: center;
	}
	...
	$.ajax({
		...
		success: function(data){
			...
			$.each(data, function(i,k){
				...
				var backgroundjob ="";
				if(k.JOB =='RC'){
					var backgroundjob = ' class="jobclass" style="background:red;color:white;"';
				}
				if(k.JOB =='GI'){
					var backgroundjob = ' class="jobclass" style="background:red;color:white;"';
				}
				if(k.JOB =="LD"){
					var backgroundjob = ' class="jobclass" style="background:yellow;color:black;"';
				}
				if(k.JOB =="DL"){
					var backgroundjob = ' class="jobclass" style="background:green;color:white;"';
				}
				if(k.JOB =="GO"){
					var backgroundjob = ' class="jobclass" style="background:green;color:white;"';
				}
				if(k.JOB =="DS"){
					var backgroundjob = ' class="jobclass" style="background:blue;color:white;"';
				}
				if(k.JOB =="MI"){
					var backgroundjob = ' class="jobclass" style="background:chocolate;color:white;"';
				}
				if(k.JOB =="MO"){
					var backgroundjob = ' class="jobclass" style="background:orange;color:white;"';
				}
				...
				htmlListYard += `data-yd-yard="${k.YD_YARD}" data-yd-block="${k.YD_BLOCK}" data-yd-slot="${k.YD_SLOT}" data-yd-row="${k.YD_ROW}" data-yd-block-name="${k.YD_BLOCK_NAME}"`;
				htmlListYard += k.JOB + ' ' + k.CONT_TYPE + ' ' + commodity + ' ' + k.CONT_SIZE + ' ' + k.ID_CLASS_CODE + ' ' + k.ID_OPERATOR + ' ' + k.NO_CONTAINER + ' ' + k.WEIGHT + 'T <br>';
				htmlListYard += '<span ' + backgroundjob + '>' + k.JOB + '</span> ' + k.NO_CONTAINER + ' ' + k.WEIGHTREAL + 'T ' + k.ID_POD + ' ' + k.PA + ' ' + truck;
				htmlListYard += '</div>';
				htmlListYard += '</a>';
			});
...
function on_chassis(){
	...
	if(!$('#btn-jobset').hasClass('btn-primary')){
		...
		yd_yard = $(elemBoxChoosed).attr('data-yd-yard');
		yd_block = $(elemBoxChoosed).attr('data-yd-block');
		yd_slot = $(elemBoxChoosed).attr('data-yd-slot');
		yd_row = $(elemBoxChoosed).attr('data-yd-row');
		yd_block_name = $(elemBoxChoosed).attr('data-yd-block-name');
...
<div style="display: inline-flex; height: 100%; position: fixed; width: 100%;">
	<div class="col4 fleft">
		...
		<div class="col12" style="display: inline-flex; ">
			<div style="background: lightblue;width: 100%; padding: 5px;text-align: center;">
				<button class="btn topbtnjob job-filter-all"onclick="setActiveBtnJobFilter(this)">
					ALL
				</button>
...
//    foreach ($jobs as $job){
				?>
				<button class="btn topbtnjob job-filter" style="background:red;color:white;" onclick="setActiveBtnJobFilter(this)">
					GI
				</button>
				<button class="btn topbtnjob job-filter" style="background:green;color:white;" onclick="setActiveBtnJobFilter(this)">
					GO
				</button>
				<button class="btn topbtnjob job-filter" style="background:blue;color:white;" onclick="setActiveBtnJobFilter(this)">
					DS
				</button>
				<button class="btn topbtnjob job-filter" style="background:yellow;color:black;" onclick="setActiveBtnJobFilter(this)">
					LD
				</button>
				<button class="btn topbtnjob job-filter" style="background:orange;color:white;" onclick="setActiveBtnJobFilter(this)">
					MO
				</button>
				<button class="btn topbtnjob job-filter" style="background:chocolate;color:white;" onclick="setActiveBtnJobFilter(this)">
					MI
				</button>
				<?php
//    }
